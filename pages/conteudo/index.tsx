...
type AnyItem = {
  id: string;
  title: string;
  kind: "newsletter" | "curso" | "ebook" | string;
  excerpt?: string;
  image?: string;
  url?: string;
};
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white/70 shadow-sm hover:shadow-md transition overflow-hidden">
      ...
      <div className="p-4 space-y-2">
        <span className="inline-flex px-2 py-1 text-xs rounded bg-emerald-50 text-emerald-700">{item.kind}</span>
        <h3 className="text-base font-semibold leading-snug">{item.title}</h3>
        {item.excerpt && <p className="text-sm text-slate-600 line-clamp-3">{item.excerpt}</p>}
        <div className="pt-2">
          {item.url ? (
            isExternal ? (
              <a href={item.url} target="_blank" rel="noopener noreferrer" className="text-emerald-700 hover:text-emerald-800 underline">Acessar</a>
            ) : (
              <Link href={item.url} className="text-emerald-700 hover:text-emerald-800 underline">Acessar</Link>
            )
          ) : null}
        </div>
      </div>
...
export default function ContentPage() {
  ...
  const newsletters = items.filter((i) => i.kind === "newsletter");
  const cursos = items.filter((i) => i.kind === "curso");
  const ebooks = items.filter((i) => i.kind === "ebook");
    ...
    <Layout title="Conteúdo — Improve" description="Conteúdo Premium da Improve: newsletters, cursos e e‑books para performance e bem‑estar.">
      <main className="mx-auto max-w-6xl px-4 py-10 space-y-16">
        <header className="space-y-2">
          ...
          <p className="text-slate-600">Newsletters, cursos e e‑books selecionados para acelerar sua evolução.</p>
        </header>

        <section id="newsletters" className="space-y-6">
          <div className="flex items-baseline justify-between">
            <h2 className="text-xl font-semibold">Newsletters</h2>
            <Link href="/#learn" className="text-emerald-700 hover:underline">Voltar à Home</Link>
          </div>
          {newsletters.length ? (
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {newsletters.map((n) => <Card key={n.id} item={n} />)}
            </div>
        ...
        <section id="cursos" className="space-y-6">
          ...
          {cursos.length ? (
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {cursos.map((c) => <Card key={c.id} item={c} />)}
            </div>
        ...
        <section id="ebooks" className="space-y-6">
          ...
          {ebooks.length ? (
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {ebooks.map((e) => <Card key={e.id} item={e} />)}
            </div>
          ) : <p className="text-slate-500">Em breve.</p>}
        </section>
      </main>
